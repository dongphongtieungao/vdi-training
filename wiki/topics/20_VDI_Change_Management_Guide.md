...
**Q1 - Scope và learning objective:** /lumi-ask riêng cho VDI Change Management Guide đã được dùng để tổng hợp phần này.
...
**Q2 - Architecture hoặc operational model:** /lumi-ask riêng cho VDI Change Management Guide đã được dùng để tổng hợp phần này.

**Synthesis:** Mô hình cần đặt trong bối cảnh hai nền tảng VDI và các lớp dependency.

**Q3 - Component deep dive và operational tasks:** /lumi-ask riêng cho VDI Change Management Guide đã được dùng để tổng hợp phần này.

**Synthesis:** Mỗi component/task phải có role, dependency, dấu hiệu lỗi, kiểm tra vận hành, evidence, risk và escalation.

**Q4 - Troubleshooting và scenario:** /lumi-ask riêng cho VDI Change Management Guide đã được dùng để tổng hợp phần này.

**Synthesis:** Troubleshooting đi theo symptom -> scope -> recent change -> layer check -> evidence -> mitigation/escalation.

**Q5 - Knowledge check, misconception và confirmation:** /lumi-ask riêng cho VDI Change Management Guide đã được dùng để tổng hợp phần này.
...
Change, patch và image management có thể ảnh hưởng hàng trăm hoặc hàng nghìn VDI nếu sai. Trong môi trường 1500 đến hơn 2000 VDI, một lỗi ở broker, gateway, image, storage, profile, identity hoặc network có thể tạo impact rộng. Engineer cần hiểu dependency, kiểm tra theo lớp và lưu evidence.

## 4. Core Concepts

- Rollback phải có trước khi change.
- Pilot giúp giảm rủi ro rollout rộng.
- Postcheck phải gồm login, launch, registration, monitoring và ticket trend.
...
Điều cần nhớ: Change, patch và image management có thể ảnh hưởng hàng trăm hoặc hàng nghìn VDI nếu sai. Khi có sự cố, hãy kiểm tra theo thứ tự: scope -> recent change -> access flow -> entitlement/resource -> broker/gateway -> agent/desktop -> identity -> hypervisor/storage/network -> monitoring trend -> escalation với evidence.
